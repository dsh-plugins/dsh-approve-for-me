...
function patchTool(def, grantMode) {
  ...
  if (spec === void 0 || typeof spec !== "object" || !Array.isArray(spec.enum)) return;
...
export function apply(ctx, config = {}) {
  ...
  const rememberStrictEscalation = (session, callId, args) => {
    if (callId === void 0 || typeof args !== "object" || args === null || typeof args.sandbox_permissions !== "string") return;
  ...
  const consumeStrictEscalation = (session, callId) => {
    if (callId === void 0) return false;
    const callIds = strictApprovedEscalations.get(session);
    if (callIds === void 0 || !callIds.delete(callId)) return false;
  ...
  const pendingApprovalIdFor = (session, req) => {
    ...
    for (let index = session.events.length - 1; index >= 0; index -= 1) {
      ...
      if (
        ...
        if (req.callId !== void 0) return event.data.id;
      }
    }
  ...
  const appendReviewLog = (req, assessment) => {
    ...
    try {
      // This plugin event is log-only and intentionally ignorable by clients
      // without the plugin, so it never changes model-visible session state.
      session.append(
        "approve-for-me/reviewed",
        {
          approvalId,
  ...
  const appendStrictReviewLog = (exec, outcome, assessment, fallbackRationale) => {
    ...
    try {
      ...
      session.append("approve-for-me/strict-reviewed", data, { ignorable: true });
    } catch (error) {
  ...
  const appendStrictReviewStarted = (exec) => {
    ...
    try {
      session.append(
        "approve-for-me/strict-review-started",
        { callId: exec.callId, toolName: exec.name },
        { ignorable: true }
      );
    } catch (error) {
  ...
  const runReview = async (req) => {
    ...
    if (llm === void 0) throw new Error("review mode requires ctx.llm (no LLM service mounted)");
   ...
   */
  const collectReviewText = async (llm, { provider, model, system, messages, signal, timeoutMs }) => {
    ...
    try {
      const parts = [];
      const stream = llm.stream({ provider, model, system, messages, signal: controller.signal, temperature: 0 });
  ...
  ctx.on(
    "tools/pre-execute",
    async (exec, next) => {
      if (exec.agent === void 0 || exec.signal?.aborted === true) return next();
      const session = exec.agent.session;
    ...
    async (req, next) => {
      ...
      if (state.mode === "review" && state.strictActive) {
        if (consumeStrictEscalation(session, req.callId)) {
          ctx.logger.info(`[approve-for-me] strict reviewer already approved ${req.toolName}${req.callId !== void 0 ? ` (call ${req.callId})` : ""}`);
          return "allowed-once";
        }
        return next();
      }
      ...
      if (state.mode === "review") {
        ...
        const toolLabel = `${req.toolName}${req.callId !== void 0 ? ` (call ${req.callId})` : ""}`;
        let decision;
        try {
      ...
      ctx.logger.info(
        `[approve-for-me] ${decision === "allowed-once" ? "approved" : "rejected"} ${req.toolName}${req.callId !== void 0 ? ` (call ${req.callId})` : ""} (mode=${state.mode}${state.presetActive ? ", preset=approve-for-me" : ""})`
      );
      return decision;
    },
    { prepend: true }
  ...
  if (presets !== void 0 && presets.presets !== void 0 && !Object.hasOwn(presets.presets, presetName)) {
  ...
  ctx.inject(["systemPrompt"], (scope) => {
    scope.systemPrompt.context({
      ...
      text: (context) => {
        ...
        switch (state.mode) {
          case "full-auto": return "Approval auto-resolution: full-auto — every approval prompt is approved automatically without asking the user.";
          case "auto": return "Approval auto-resolution: auto — approval prompts matching the configured rules are approved automatically, prompts matching deny rules are rejected, and anything else is presented to the user.";
          case "never": return "Approval auto-resolution: never — every approval prompt is rejected automatically; do not request sandbox escalation (do not set `sandbox_permissions`).";
          default: return "Approval auto-resolution: off — approval prompts are presented to the user.";
        }
      }
    });
  });
