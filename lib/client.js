...
loader.load({
    ...
    factory: (require) => {
         ...
         */
        const ui = require("@dsh-plugin/dsh-loader/client");
        // ------------------------------------------------------------------
        ...
        const css = ".afmRoot{align-items:center;min-width:0;height:24px;display:flex}" +
        ...
        function ApprovalStatusNode(props) {
            if (props.node === undefined || props.node === null || props.node.data === undefined)
                return null;
            const status = props.node.data;
            const pending = status.outcome === null;
            const flow = status.flow === "strict" ? "strict" : "approval";
            const reviewText = !pending && status.review !== null && status.review !== undefined
        ...
        const approvalStatusDefinition = {
            ...
            match(event) {
                ...
                if (event.type === "hook/result" && event.data?.hook === "approve-for-me/review") {
                    return { id: String(event.data?.approvalId), role: "update" };
                }
                if (event.type === "hook/invoked" && event.data?.hook === "approve-for-me/strict-review") {
                    return { id: "strict:" + String(event.data?.callId), role: "start" };
                }
                if (event.type === "hook/result" && event.data?.hook === "approve-for-me/strict-review") {
                    return { id: "strict:" + String(event.data?.callId), role: "update" };
                }
                return null;
            },
        ...
        const plugin = {
            ...
            inject: ["conversationEvents", "slots"],
            apply(ctx) {
