/**
 * Pure decision logic for the approve-for-me plugin.
 *
 ...
 * reasoned about without the harness. The cordis plugin in `index.js` only
 ...
 * @param input - the request facts:
 *   - `mode`: plugin config mode (`off` | `auto` | `full-auto` | `never`).
 *   - `presetActive`: the session currently runs the `approve-for-me`
 *     permission preset (user chose "替我同意" in the UI / `/permission`).
 *   - `approve` / `deny`: compiled regex patterns used by `auto` mode.
 *   - `reason`, `toolName`, `commandText`: request facts for matching.
...
export function isApproveForMeRequest(args) {
  return typeof args === "object" && args !== null && args.sandbox_permissions === APPROVE_FOR_ME_MODE;
}
...
export function collectReviewTranscript(events = [], limits = REVIEW_LIMITS) {
  ...
  for (const event of events) {
    switch (event.type) {
      case "user/message":
        push("user", contentText(event.data?.content));
        break;
      ...
      case "tool/call": {
        ...
        if (data?.callId !== void 0) toolNamesByCallId.set(data.callId, data.name ?? "tool");
        push(`tool ${data?.name ?? "tool"} call`, data?.arguments ?? "");
        break;
      }
      case "tool/result": {
        const data = event.data;
        const name = data?.message?.callId !== void 0 ? toolNamesByCallId.get(data.message.callId) : void 0;
        push(`tool ${name ?? "tool"} result`, contentText(data?.message?.content));
        break;
      }
      default:
        break;
...
export function parseGuardianAssessment(text) {
  ...
  return {
    ...
    userAuthorization:
      typeof payload.user_authorization === "string" ? payload.user_authorization : "unknown",
    rationale:
...
export function createDenialTracker() {
  ...
  return {
    ...
    record(key, turn, denied, limits = {}) {
      ...
      if (entry === void 0 || entry.turn !== turn) {
    ...
    tripped(key, turn) {
      ...
      return entry !== void 0 && entry.turn === turn && entry.tripped;
    }
  };
}
