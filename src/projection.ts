...
const init = (): ApprovalStatusState => ({ pending: null, last: null });
...
function apply(state: ApprovalStatusState, event: SessionEvent): ApprovalStatusState {
  switch (event.type) {
    case "approval/asked":
      return {
        pending: {
...
const view = (state: ApprovalStatusState): ApprovalStatusState => state;
...
export function registerApprovalProjection(
  inject: (services: readonly string[], callback: (scope: ProjectionScope) => void) => void,
): void {
  inject(["sessionProjections"], (projectionCtx) => {
    const registry = (projectionCtx as unknown as { sessionProjections?: ProjectionScope["sessionProjections"] })
      .sessionProjections;
