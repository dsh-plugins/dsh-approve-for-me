...
export const MODES = ["off", "auto", "full-auto", "never", "review"] as const;

export type Mode = (typeof MODES)[number];

/** The real sandbox modes `approve-for-me` may grant on the user's behalf. */
export const GRANT_MODES = ["workspace-write", "danger-full-access"] as const;
 ...
 */
export function compilePatterns(sources: readonly string[] | undefined): CompiledPatterns {
  const out: RegExp[] = [];
 ...
 * the session log.
 */
export function buildMatchText({ toolName = "", reason = "", commandText = "" }: MatchFacts = {}): string {
...
export function decideApproval({
  ...
  toolName = "",
  commandText = "",
}: DecideApprovalInput = {}): ApprovalDecision {
  if (mode === "never") return "rejected";
  if (presetActive || mode === "full-auto") return "allowed-once";
  if (reason.includes(APPROVE_FOR_ME_MARKER)) return "allowed-once";
  if (mode === "auto") {
    const text = buildMatchText({ toolName, reason, commandText });
    if (matchesAny(text, deny)) return "rejected";
    if (matchesAny(text, approve)) return "allowed-once";
    return null;
  }
 ...
 */
export function isApproveForMeRequest(args: unknown): boolean {
  return (
    typeof args === "object" &&
    args !== null &&
    (args as Record<string, unknown>).sandbox_permissions === APPROVE_FOR_ME_MODE
  );
}
 ...
 */
export function rewriteApproveForMe(
 ...
 */
export function renderReviewSystemPrompt(policyAdditions?: string): string {
  const additions =
    typeof policyAdditions === "string" && policyAdditions.trim() !== ""
      ? policyAdditions
...
/** One normalized transcript entry before rendering. */
interface TranscriptEntry {
  kind: string;
  text: string;
}

/** A rendered transcript line with inclusion bookkeeping. */
...
function truncateEntry(text: string, cap: number): { text: string; truncated: boolean } {
  if (text.length <= cap) return { text, truncated: false };
  const marker = "<truncated omitted_approx_tokens=\"...\" />";
  const markerBytes = marker.length;
  if (cap <= markerBytes) return { text: marker, truncated: true };
...
/** A DSH message content block (the text-bearing variant). */
 ...
 */
export function collectReviewTranscript(
  events: readonly TranscriptEvent[] = [],
  limits: ReviewLimits = REVIEW_LIMITS,
): { lines: string[]; omitted: boolean } {
  const entries: TranscriptEntry[] = [];
  const toolNamesByCallId = new Map<string, string>();
  const push = (kind: string, text: unknown): void => {
    const trimmed = typeof text === "string" ? text.trim() : "";
    if (trimmed !== "") entries.push({ kind, text: trimmed });
  };
  for (const event of events) {
    ...
    switch (event.type) {
      ...
      case "tool/call": {
        ...
        if (callId !== undefined) toolNamesByCallId.set(String(callId), String(data.name ?? "tool"));
        push(`tool ${String(data.name ?? "tool")} call`, data.arguments ?? "");
        break;
      }
      case "tool/result": {
        const message = data.message as { callId?: unknown; content?: unknown } | undefined;
  ...
  if (entries.length === 0) return { lines: [], omitted: false };

  const rendered: RenderedEntry[] = entries.map((entry, index) => {
  ...
  const included = new Array<boolean>(rendered.length).fill(false);
  let messageChars = 0;
  let toolChars = 0;
  ...
  const take = (index: number): void => {
    ...
    if (current + rendered[index].text.length > budget) return;
    included[index] = true;
    if (isTool) toolChars += rendered[index].text.length;
    else messageChars += rendered[index].text.length;
  };

  if (userIndices.length > 0) take(userIndices[0]);
  if (userIndices.length > 1) take(userIndices[userIndices.length - 1]);
  for (let index = userIndices.length - 1; index >= 0; index -= 1) take(userIndices[index]);

  let retainedNonUser = 0;
  for (let index = rendered.length - 1; index >= 0; index -= 1) {
    if (rendered[index].kind === "user") continue;
    if (retainedNonUser >= limits.recentEntryLimit) continue;
    take(index);
    if (included[index]) retainedNonUser += 1;
  }
...
export function renderReviewUserPrompt({
  ...
  sessionId = "unknown",
  actionJson = "{}",
}: ReviewUserPromptInput = {}): string {
  const transcript =
    transcriptLines.length === 0 ? "<no retained transcript entries>" : transcriptLines.join("\n");
  const parts = [
  ...
  if (omitted) parts.push("Some conversation entries were omitted.");
  parts.push(
    "The DSH agent has requested the following action:",
 ...
 */
export function parseGuardianAssessment(text: unknown): GuardianAssessment | null {
  ...
  return {
    outcome,
    riskLevel: typeof payload.risk_level === "string" ? (payload.risk_level as RiskLevel) : outcome === "allow" ? "low" : "high",
    userAuthorization:
      typeof payload.user_authorization === "string"
...
/** Per-session denial-tracking state keyed by turn. */
...
export function createDenialTracker(): DenialTracker {
  const byKey = new Map<unknown, DenialEntry>();
  return {
    /**
    ...
    record(key: unknown, turn: number, denied: boolean, limits: CircuitLimits = {}): CircuitVerdict {
      ...
      if (entry.tripped) return "tripped";
      if (denied) {
        entry.consecutive += 1;
      ...
      while (entry.recent.length > window) entry.recent.shift();
